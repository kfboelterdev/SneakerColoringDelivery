...
namespace pl.ayground {
    ...
    public class DrawableTextureContainer {
        ...
        // Naka: Changed private --> public for IsInImageBounds()
        ...
        private Color32[] getDirtyAreaAsColorArray() {
            ...
            //Debug.Log($"Source Width {w} \nSource Height {h} \nretvalSize {retVal.Length}\nstartingpixelPos {startingPixelPosition}");
            for (int row = 0; row < h; row++) {
                for (int cell = 0; cell < w; cell++) {
                    if (row * w + cell >= retVal.Length) {
                        //Debug.Log("Tryng to Access retVal outside of bounds");
                        continue;
                    }
                    if (startingPixelPosition + dirtyArea.x + cell + row * this.width >= pixels.Length) {
                        //Debug.Log("Tryng to Access pixels outside of bounds");
                        continue;
                    }
        ...
        public Texture2D getTexture() {
            if (isDirty) {
                if (this.dirtyArea.width > 0) {
                    //Debug.Log($"{dirtyArea.ToString()}");
                    MarkFullAsDirty(); //
                    //texture.SetPixels32(dirtyArea.x, dirtyArea.y, dirtyArea.width, dirtyArea.height, getDirtyAreaAsColorArray());
                    texture.SetPixels32(0, 0, dirtyArea.width, dirtyArea.height, getDirtyAreaAsColorArray());
                    texture.Apply();
                    isDirty = false;
                    dirtyArea = new TextureRegion();
                }
